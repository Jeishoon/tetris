        ...
        for i in range(4):
            for j in range(4):
                ...
                if p in self.preview_piece.get_state():
                    ...
                    self.screen.blit(colours_ct[self.preview_piece.colour], [xpos, ypos])
...
class Classic_Tetris_DS(Classic_Tetris):

    def __init__(self,height,width,x,y,screen):
        super().__init__(height,width,x,y,screen)
        #Method for generating initial garbage
        self.field = []
        i = 0
        while i < height:
            new_line = []
            for j in range(width):
                #only adds lines up to halfway down the grid
                if i >= 10:
                    #adds 0 or 8 to line dependent on probabilities 
                    new_line.append(random.choices([0,8],[0.5,0.5])[0])
                #adds spaces from the middle of the grid upwards
                else:
                    new_line.append(0)
            #makes sure a space is present in the line first
            if 0 in new_line:
    ...
    #checks for the presence of garbage on the field
    #to decide if the game is to end or not
